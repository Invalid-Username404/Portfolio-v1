...
const WorkSampleCard = ({ id, sample }: WorkSampleCardProps) => {
  const { name, description, tags, source_code_link, image, deployment } =
    sample;

  return (
    <motion.div variants={fadeIn("up", "spring", id * 0.5, 0.75)}>
      <Tilt className="bg-slate-900  p-1 rounded-2xl  h-full sm:w-[340px] w-full  ">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {tags.map((tag, id) => (
            <div
              key={`${tag.name}-${id}-yep`}
              className="bg-[#dcfce7] rounded-full p-1"
            >
        ...
        <div className=" h-[5vh] hover:text-[#6d28d9]  p-2 text-cyan-400">
          <p>
            <a target="_blank" href={`${deployment.link}`}>
              Deployment link: {`${deployment.on}`}
            </a>{" "}
          </p>
        </div>
      </Tilt>
  ...
  return (
    <>
      ...
      <div className="w-full flex">
        ...
        >
          Check the source code by pressing the Github icons or see visit my
          <a
            rel="noopener"
            target="_blank"
            className="text-[#6d28d9]"
            href="https://github.com/Invalid-Username404"
          >
            {" "}
            Github{" "}
          </a>
      ...
      <div className="mt-20 flex justify-center align-top flex-row flex-wrap gap-7  ">
        {projects.map((sample, id) => (
          <WorkSampleCard key={`sample-${id}`} id={id} sample={sample} />
        ))}
      </div>
    </>
  );
